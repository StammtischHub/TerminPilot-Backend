...
/**
 * Service implementation for interacting with Google Calendar.
 *
 * This service provides methods to fetch and write appointments to a specific Google Calendar
...
final class GoogleCalendarService(
     ...
     */
    private final fun parseAppointments(events: List<Event>): List<Appointment> {
     ...
     */
    private final fun parseEvent(appointment: Appointment): Event? = Event()
     ...
     */
    private final fun toLocalDateTime(googleDateTime: DateTime): LocalDateTime {
        return LocalDateTime.ofInstant(Instant.ofEpochMilli(googleDateTime.value), ZoneId.systemDefault())
    }
     ...
     */
    private final fun toGoogleDateTime(localDateTime: LocalDateTime): DateTime {
        val instant = localDateTime.atZone(ZoneId.systemDefault()).toInstant()
        return DateTime(instant.toEpochMilli())
    }

    /**
     * Fetches appointments from the Google Calendar for the specified time range.
     *
    ...
    override fun getCalendarForTimespan(start: LocalDateTime, end: LocalDateTime): List<Appointment> {
        val upcomingEvents = calendarClient.events().list(calendarId)
    ...
    /**
     * Inserts a new appointment into the Google Calendar.
     *
